...
class B_UEGMF_Experiment:
    ...
    def _batch_process_unlearnable_examples(self, dem_generator: DynamicErrorMinimizingNoise,
        ...
        total_batches = min(5, len(train_loader)) 
    ...
    def comprehensive_evaluation(self, clean_model: nn.Module, 
        ...
        test_images = torch.cat(test_images, dim=0)[:50]  
    ...
    def setup_access_control(self, user_addresses: List[str]) -> tuple:
        ...
        logger.info(f"Adding {len(user_addresses)} users to whitelist")
        for i, user in enumerate(user_addresses):
            merkle_manager.add_leaf(user)
        
        # Build Merkle tree
        root = merkle_manager.build_tree()
        logger.info(f"Merkle tree construction complete, root hash: {root[:16]}...")
        
        # Verify Merkle tree construction success
        tree_size = len(merkle_manager.leaves)
        logger.info(f"Merkle tree contains {tree_size} leaf nodes")
        ...
        logger.info(f"Access control setup complete - Merkle root: {root[:16]}...")
    ...
    def test_access_control(self, dataset_hash: str, merkle_manager) -> Dict[str, Any]:
        ...
        user_accounts = list(self.blockchain.accounts.keys())[3:8]
        
        access_results = {}
        
        for i, user in enumerate(user_accounts):
            try:
                logger.info(f"Testing access permissions for user {user[:8]}...")
                
                # Ensure user is in Merkle tree
                if not merkle_manager.has_leaf(user):
                    logger.warning(f"User {user[:8]}... not in whitelist, adding to whitelist")
                    merkle_manager.add_leaf(user)
                    # Rebuild Merkle tree
                    root = merkle_manager.build_tree()
                    # Update Merkle root in contract
                ...
                if proof:
                    logger.info(f"Generated Merkle proof for user {user[:8]}...")
                    
                    # Verify proof
                    is_valid = proof.verify()
                    logger.info(f"Merkle proof verification result: {is_valid}")
                    ...
                    if is_valid:
                        ...
                        logger.info(f"User {user[:8]}... authorized, Token ID: {result['token_id']}")
                        ...
                        logger.warning(f"User {user[:8]}... Merkle proof invalid")
                    ...
                    logger.error(f"Cannot generate Merkle proof for user {user[:8]}...")
                ...
                logger.error(f"User {user[:8]}... access control test failed: {e}")
