...
class Bottleneck(nn.Module):
    def __init__(self, in_planes, growth_rate):
        ...
        self.dropout = nn.Dropout(p=0.2)  # Add dropout after the bottleneck block

    def forward(self, x):
        out = self.conv1(F.relu(self.bn1(x)))
        out = self.conv2(F.relu(self.bn2(out)))
        out = self.dropout(out)  # Apply dropout to the output of the second convolution
...
if __name__ == '__main__':
    ...
    print(model)
    x = torch.randn(1, 3, 32, 32)  # CIFAR-10的输入图像
    output = model(x)
    print("Output size:", output.size())  # 输出预测结果的形状
